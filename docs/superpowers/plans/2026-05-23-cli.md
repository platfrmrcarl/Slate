...
**Goal:** Ship the `wpkiller` CLI listed in spec §17.4 — a single executable that an operator runs against either a local install (talks to the dev server) or a deployed install (talks to a Cloud Run URL with an admin token). Commands: `setup`, `user create`, `user reset-password`, `theme install`, `theme activate`, `plugin install`, `import`, `export`, `backup`, `migrate`, `shell`. Implemented as a Node CLI under `packages/cli/` that's published as `@wpkiller/cli` but is also usable locally via `pnpm cli ...`.

**Architecture:** Two transport paths: (1) **local** — when run against the same repo (e.g., during dev) the CLI imports services directly via `tsx` and operates on the configured `DATABASE_URL`; (2) **remote** — when invoked against a deployed URL, it uses a bearer token (stored under `~/.config/wpkiller/credentials.json`) and hits a small admin-only `/api/cli/*` surface. The CLI picks transport via `WPK_URL` env var or `--url` flag (remote) versus presence of a local `package.json` at the repo root (local).
...
| `package.json` | **MODIFY** — root script `"cli": "pnpm --filter @wpkiller/cli start --"` |
...
```json
"scripts": {
  "cli": "pnpm --filter @wpkiller/cli start --"
}
```
...
const { saveCredentials, loadCredentials, clearCredentials, credentialsPath } = await import("./credentials");
...
```bash
pnpm --filter @wpkiller/cli test packages/cli/src/credentials.test.ts
```
...
afterAll(async () => {
  if (!HAS_DB) return;
  for (const id of cleanup) await db().delete(users).where(sql`${users.id} = ${id}`);
  await closeDb();
});
...
describe.runIf(HAS_DB)("admin-token", () => {
  it("issueAdminToken returns a token string; only the hash is stored", async () => {
    ...
    const rows = await db().select().from(adminTokens).where(sql`${adminTokens.userId} = ${u!.id}`);
...
export async function issueAdminToken(input: IssueInput): Promise<{ token: string; tokenHash: string }> {
...
export async function verifyAdminToken(token: string): Promise<User | null> {
  ...
  const row = rows[0];
  if (!row) return null;
  await db().update(adminTokens).set({ lastUsedAt: sql`now()` }).where(eq(adminTokens.tokenHash, tokenHash));
  return row.user;
}

export async function revokeAdminToken(token: string): Promise<void> {
  await db().delete(adminTokens).where(eq(adminTokens.tokenHash, hashToken(token)));
}
```
...
```bash
pnpm --filter @wpkiller/cli test packages/cli/src/transport.test.ts
```
...
describe("POST /api/cli/users/create", () => {
  ...
  it("returns 403 when token belongs to non-admin user", async () => {
    ...
    const res = await POST(req({ email: "a@b", displayName: "A", password: "x".repeat(12), role: "author" }, { authorization: "Bearer wpk_t" }));
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (err instanceof EmailInUseError) return NextResponse.json({ error: "email already in use" }, { status: 409 });
    throw err;
  }
}
```
...
export function userCommand(opts: TransportOpts): Command {
  ...
  cmd
    ...
    .action(async (email: string, sub: { role: string; displayName?: string; password?: string }) => {
      const transport = resolveTransport(opts);
      const displayName =
        sub.displayName ?? (await p.text({ message: "Display name", placeholder: "Jane Doe" })) as string;
      const password =
      ...
      if (transport === "remote") {
        const { id } = await remoteRequest<{ id: string }>("POST", "/api/cli/users/create", payload, opts);
  ...
  cmd
    ...
    .action(async (email: string) => {
      const { url } = await remoteRequest<{ url: string }>("POST", "/api/cli/users/reset-password", { email }, opts);
...
```bash
pnpm --filter @wpkiller/cli test packages/cli/src/commands/user.test.ts
  ...
  cmd
    ...
    .action(async (source: string) => {
      ...
      if (source.endsWith(".zip")) {
        ...
        await sh(`git clone --depth 1 ${JSON.stringify(source)} ${JSON.stringify(path.join(dest, slug))}`);
      }
      console.log(pc.green("Theme cloned/unzipped. Run `pnpm build` and redeploy to make it active in the registry."));
    });
...
export async function GET(req: Request): Promise<Response> {
  ...
  return NextResponse.json({ status: row.status, errorMessage: row.errorMessage, result: row.result });
}
```
...
async function poll(id: string, opts: TransportOpts): Promise<{ status: string; result?: { sizeBytes?: number } }> {
  for (let attempt = 0; attempt < 600; attempt++) {
    const r = await remoteRequest<{ status: string; result?: { sizeBytes?: number } }>("GET", `/api/cli/exports?id=${id}`, undefined, opts);
...
export function exportCommand(opts: TransportOpts, includeDb: boolean): Command {
  ...
  return new Command(name)
    ...
    .description(includeDb ? "Trigger a full export (+ db dump) and download it" : "Trigger an export and download it")
    .action(async (outputPath: string) => {
      const { id } = await remoteRequest<{ id: string }>("POST", "/api/cli/exports", { includeDb }, opts);
      ...
      const dl = await fetch(
        (opts.url ?? process.env.WPK_URL!).replace(/\/$/, "") + `/api/export/${id}/download`,
        { headers: { authorization: `Bearer ${opts.token ?? process.env.WPK_TOKEN}` } },
      );
...
export function importCommand(opts: TransportOpts): Command {
  return new Command("import")
    ...
    .action(async (source: string, file: string) => {
      ...
      const url = (opts.url ?? process.env.WPK_URL!).replace(/\/$/, "") + `/api/import/${source}`;
...
```bash
pnpm --filter @wpkiller/cli build
```
...
WPK_URL=http://localhost:3000 WPK_TOKEN=wpk_... pnpm cli theme activate wpk-default
```
