...
```json
{
  ...
  "slug": "wpk-default",
  "version": "1.0.0",
  "description": "Polished baseline theme bundled with WordPressKiller.",
  ...
  "customizations": [
    ...
    { "key": "fontDisplay", "type": "font", "label": "Display font", "default": "Inter, system-ui, sans-serif" },
    { "key": "fontBody", "type": "font", "label": "Body font", "default": "Inter, system-ui, sans-serif" },
    { "key": "radius", "type": "select", "label": "Corner radius", "default": "md",
      "options": [
    ...
    { "key": "containerWidth", "type": "select", "label": "Container width", "default": "3xl",
      "options": [
    ...
    { "key": "footerText", "type": "text", "label": "Footer text", "default": "Made with WordPressKiller." }
  ],
  "templates": {
    "page": "page",
    "post": "post",
...
export interface ThemeModule {
  ...
  primitives: {
    ...
    Image: ComponentType<{ media: string; alt?: string; caption?: string; size?: "small" | "medium" | "full" }>;
  };
  templates: {
...
afterAll(async () => {
  ...
  for (const id of cleanupIds) {
    await db().delete(activeTheme).where(sql`${activeTheme.themeId} = ${id}`);
    await db().delete(themes).where(sql`${themes.id} = ${id}`);
  }
  await closeDb();
});
...
export async function activateTheme(themeId: string) {
  ...
  const defaults = defaultCustomizationFor(
    themeManifestSchema.parse(theme.manifest),
  );
  const [row] = await db()
    .insert(activeTheme)
...
vi.mock("./registry", () => ({ resolveThemeModule: (...a: unknown[]) => resolveThemeModule(...a) }));
...
afterAll(async () => {
  if (!HAS_DB) return;
  await db().delete(activeTheme).where(sql`true`);
  await db().delete(themes).where(sql`${themes.slug} = 'wpk-default'`);
  await closeDb();
});
...
async function run(): Promise<void> {
  ...
  if (!active) {
    ...
    logger().info({ themeId: themeRow.id }, "themes:registered wpk-default (kept existing active theme)");
  }
}
```
...
export function Layout({ children, tokens }: { children: ReactNode; tokens: Record<string, unknown> }) {
...
export default function ArchiveTemplate({ children, title }: { children: ReactNode; title: string }) {
  return (
...
describe("resolveThemeContext", () => {
  ...
  it("returns shape with primitives and tokens", async () => {
    getActiveTheme.mockResolvedValue({
      ...
      tokens: { primary: "#000" },
      module: {
        primitives: { Heading: () => null, Paragraph: () => null, Button: () => null, Hero: () => null, Image: () => null },
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <html lang="en">
      <body>{theme ? <theme.Layout tokens={theme.tokens}>{children}</theme.Layout> : children}</body>
    </html>
  );
}
```
...
describe("activateThemeAction", () => {
  it("requires admin+", async () => {
    ...
    const r = await activateThemeAction(undefined, fd({ themeId: "11111111-1111-1111-1111-111111111111" }));
...
export async function customizeThemeAction(
  ...
  try {
    await setCustomization(parsed.data.themeId, overrides as Record<string, string | number | boolean>);
  } catch (err) {
  ...
  return (
    <main className="p-6">
      <header className="mb-6 flex items-center justify-between">
        ...
        <Link href="/admin/themes/install" className="rounded bg-black px-3 py-1.5 text-sm text-white">
          Install theme
        </Link>
      </header>
      <ul className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {themesList.map((t) => (
          <li key={t.id} className="rounded border p-3">
            ...
            <p className="text-xs text-gray-500">v{t.version} · {t.slug}</p>
    ...
    <form action={action} className="space-y-4">
      ...
      <input
        ...
        value={JSON.stringify(
          Object.fromEntries(
            Object.entries(local).map(([k, v]) => [k, v === "true" ? true : v === "false" ? false : v]),
          ),
        )}
      />
...
export default async function InstallThemePage() {
  ...
  return (
    <main className="prose mx-auto max-w-2xl p-6">
      ...
      <p>
        Use the CLI to scaffold a theme directory from a Git URL or a local folder:
      </p>
      <pre>
